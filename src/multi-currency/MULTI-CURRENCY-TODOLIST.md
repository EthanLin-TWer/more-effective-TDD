...
永远先行。他讲写测试会减少压力，毛线，我看到的情况都是写测试增加压力，不会写，写不好反而增加维护成本，最后导致都是后面写，不写。

他讲写测试可以帮助你思考设计，控制需求。后者感觉是 tasking 做的事，前者究竟多大程度上能驱动你思考设计，有待观察。

**断言先行**。

这个讲的很好。他从什么时候写断言开始发散起，其实是一种 API 式的思维：我「希望」这东西是怎么样的。这有个人的审美在里面，有个人的设计能力在里面，同样，是主观的。他发了几个问：

* 应该在什么地方开始构建一个系统？想想系统完成后，它能做到什么事
* 应该在什么地方开始写一些业务功能？想想功能完成后，你希望什么测试能够通过
* 应该在什么地方开始写一个测试？想想测试完成后，你希望什么断言能够通过

从「你想要什么」的角度，开始你的测试，开始你的断言。当你在写测试的时候，尽管它已经同实现解耦开来，但是依然有很多事情是你写测试时会需要考虑的，比如说：

* （设计范畴）这个功能应该写在什么地方？是修改已有的方法，还是在已有的类增加新的方法，还是在新的地方抄个方法，还是创建一个新的类？
* 应该怎么命名这个你想要的东西（类、对象等）？
* 你如何拿到这个东西的输出结果呢？
* 正确的输出结果是什么呢？
* 这个测试，是否在呼喊你实现其他的测试？

一般来说，「如何获得输出结果（API）」和「正确的结果是什么」是写好断言的两个发问，可以从断言出发，逆推补全尚没有但你需要的那些元素。

**测试数据的选择**。

是选择逻辑等同的测试数据组？还是使用真实数据？还是怎么样。在测试数据中，尽量让输入和输出的关系变得明显，这样测试会非常非常易读。权衡好硬编码常量和变量的应用。等等。这也是一块功夫需要探索。
